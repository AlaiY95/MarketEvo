// app/dashboard/scalp-trading/page.tsx
'use client';
...
export default function ScalpTrading() {
  ...
  const [tradeSetup, setTradeSetup] = useState<TradeSetup>({
    ...
    riskPerTrade: 0.5, // Lower risk for scalping
    entryPrice: 0,
    stopLoss: 0,
    takeProfit: 0
  });
  ...
  useEffect(() => {
    const savedResult = localStorage.getItem('lastScalpAnalysis');
    const savedParsed = localStorage.getItem('lastScalpParsed');
    const savedImage = localStorage.getItem('lastScalpImage');
    const savedSetup = localStorage.getItem('scalpTradeSetup');
  ...
  useEffect(() => {
    localStorage.setItem('scalpTradeSetup', JSON.stringify(tradeSetup));
  }, [tradeSetup]);

  // Save analysis results to localStorage whenever they change
  useEffect(() => {
    if (result) {
      localStorage.setItem('lastScalpAnalysis', JSON.stringify(result));
    }
  }, [result]);
  ...
  useEffect(() => {
    if (parsedAnalysis) {
      ...
      localStorage.setItem('lastScalpParsed', JSON.stringify(parsedAnalysis));
    }
  }, [parsedAnalysis]);

  const analyzeScalpChart = async (file: File) => {
    if (!session?.user?.id) {
      return;
    }
    ...
    try {
      ...
      reader.onload = () => {
        ...
        localStorage.setItem('lastScalpImage', imageData);
      };
      reader.readAsDataURL(file);

      // Create FormData and send to API with scalp trading context
      const formData = new FormData();
      formData.append('image', file);
      formData.append('tradingStyle', 'scalp');
  ...
  const clearAll = () => {
    ...
    localStorage.removeItem('lastScalpAnalysis');
    localStorage.removeItem('lastScalpParsed');
    localStorage.removeItem('lastScalpImage');
  };
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div>
                ...
                <div className="relative group">
                  ...
                  <div className="absolute left-0 top-6 w-80 bg-white border border-gray-200 rounded-lg shadow-lg p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="font-medium text-red-700 mb-2">Optimal Timeframes</h4>
                        <ul className="text-sm text-red-600 space-y-1">
                          <li>• 1-minute charts (primary)</li>
                          <li>• 5-minute charts</li>
                          <li>• 15-minute for context</li>
                        </ul>
                      </div>
                      <div>
                        <h4 className="font-medium text-red-700 mb-2">Key Strategies</h4>
                        <ul className="text-sm text-red-600 space-y-1">
                          <li>• Quick momentum plays</li>
                          <li>• Level-to-level trading</li>
              ...
              <p className="text-gray-600">
                Ultra-fast analysis for quick profit opportunities (1m-15m timeframes)
              </p>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="bg-red-100 px-4 py-2 rounded-lg">
                <span className="text-red-800 font-medium">1m/5m Charts Recommended</span>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-lg shadow border">
              <div className="p-6">
                ...
                <div className="text-center mb-6">
                  <ImageUpload onUpload={analyzeScalpChart} loading={loading} />
                </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-blue-700 text-sm">
                        Our AI analyzes ultra-short timeframe patterns for rapid scalping opportunities. 
                        Best with 1m-5m charts showing precise entry and exit levels with tight spreads.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {loading && (
                  <div className="bg-red-50 rounded-lg p-6 text-center">
                    ...
                    <p className="text-red-700 font-medium">Analyzing scalp trading opportunity...</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Right Column - Trade Setup & Scalp Trading Info */}
          <div className="space-y-6">
              ...
              <div className="p-6">
                ...
                {activeTab === 'history' && (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">Scalp Trading History</h3>
                    <p className="text-gray-600 text-sm mb-4">
                      Your scalp trading analysis history will appear here
                    </p>
                    <button 
            ...
            {/* Scalp Trading Rules */}
            <div className="bg-white rounded-lg shadow border p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Scalp Trading Rules</h3>
              <div className="space-y-3">
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Exit quickly - don't hold overnight</span>
                </div>
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Use tight stops (2-5 pips/cents)</span>
                </div>
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Focus on high-volume periods</span>
                </div>
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Trade during market hours only</span>
                </div>
                <div className="flex items-start">
                  <span className="text-red-500 mr-2 mt-1">×</span>
                  <span className="text-sm text-gray-700">Don't average down on losses</span>
                </div>
                <div className="flex items-start">
                  <span className="text-red-500 mr-2 mt-1">×</span>
                  <span className="text-sm text-gray-700">Avoid low-liquidity instruments</span>
                </div>
              </div>
            </div>
          </div>
