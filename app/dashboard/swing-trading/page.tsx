// app/dashboard/swing-trading/page.tsx
'use client';
...
export default function SwingTrading() {
  ...
  useEffect(() => {
    const savedResult = localStorage.getItem('lastSwingAnalysis');
    const savedParsed = localStorage.getItem('lastSwingParsed');
    const savedImage = localStorage.getItem('lastSwingImage');
    const savedSetup = localStorage.getItem('swingTradeSetup');
  ...
  useEffect(() => {
    localStorage.setItem('swingTradeSetup', JSON.stringify(tradeSetup));
  }, [tradeSetup]);

  // Save analysis results to localStorage whenever they change
  useEffect(() => {
    if (result) {
      localStorage.setItem('lastSwingAnalysis', JSON.stringify(result));
    }
  }, [result]);
  ...
  useEffect(() => {
    if (parsedAnalysis) {
      ...
      localStorage.setItem('lastSwingParsed', JSON.stringify(parsedAnalysis));
    }
  }, [parsedAnalysis]);

  const analyzeSwingChart = async (file: File) => {
    if (!session?.user?.id) {
      return;
    }
    ...
    try {
      ...
      reader.onload = () => {
        ...
        localStorage.setItem('lastSwingImage', imageData);
      };
      reader.readAsDataURL(file);

      // Create FormData and send to API with swing trading context
      const formData = new FormData();
      formData.append('image', file);
      formData.append('tradingStyle', 'swing');
  ...
  const clearAll = () => {
    ...
    localStorage.removeItem('lastSwingAnalysis');
    localStorage.removeItem('lastSwingParsed');
    localStorage.removeItem('lastSwingImage');
  };
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center space-x-2 mb-2">
                ...
                <h1 className="text-3xl font-bold text-gray-900">Swing Trading</h1>
                {/* Info tooltip */}
                <div className="relative group">
                  ...
                  <div className="absolute left-0 top-6 w-80 bg-white border border-gray-200 rounded-lg shadow-lg p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="font-medium text-blue-700 mb-2">Optimal Timeframes</h4>
                        <ul className="text-sm text-blue-600 space-y-1">
                          <li>• Daily charts (primary)</li>
                          <li>• 4-hour charts</li>
                          <li>• Weekly for context</li>
                        </ul>
                      </div>
                      <div>
                        <h4 className="font-medium text-blue-700 mb-2">Key Strategies</h4>
                        <ul className="text-sm text-blue-600 space-y-1">
                          <li>• Trend following</li>
                          <li>• Support/resistance breaks</li>
                          <li>• Chart pattern setups</li>
                        </ul>
                      </div>
                    </div>
                  </div>
            ...
            {/* Usage Indicator and Recommendations */}
            <div className="flex items-center space-x-4">
              <div className="bg-blue-100 px-4 py-2 rounded-lg">
                <span className="text-blue-800 font-medium">Daily/4H Charts Recommended</span>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-lg shadow border">
              <div className="p-6">
                ...
                <div className="text-center mb-6">
                  <ImageUpload onUpload={analyzeSwingChart} loading={loading} />
                </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-blue-700 text-sm">
                        Our AI analyzes swing trading patterns and trends for multi-day to multi-week opportunities. 
                        Best with daily/4H charts showing clear trend structure and key levels.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {loading && (
                  <div className="bg-blue-50 rounded-lg p-6 text-center">
                    ...
                    <p className="text-blue-700 font-medium">Analyzing swing trading opportunity...</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Right Column - Trade Setup & Swing Trading Info */}
          <div className="space-y-6">
              ...
              <div className="p-6">
                ...
                {activeTab === 'history' && (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">Swing Trading History</h3>
                    <p className="text-gray-600 text-sm mb-4">
                      Your swing trading analysis history will appear here
                    </p>
                    <button 
            ...
            <div className="bg-white rounded-lg shadow border p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Swing Trading Metrics</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b">
                  <span className="text-gray-600">Hold Period</span>
                  <span className="font-medium">2-30 days</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b">
                  <span className="text-gray-600">Risk/Reward</span>
                  <span className="font-medium text-blue-600">1:2 - 1:4</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b">
                  <span className="text-gray-600">Win Rate Target</span>
                  <span className="font-medium text-green-600">45-55%</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-gray-600">Position Size</span>
                  <span className="font-medium">1-3% risk</span>
                </div>
              </div>
            </div>
            ...
            {/* Swing Trading Rules */}
            <div className="bg-white rounded-lg shadow border p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Swing Trading Rules</h3>
              <div className="space-y-3">
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Follow the trend direction</span>
                </div>
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Wait for pullbacks in trends</span>
                </div>
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Use larger stop losses</span>
                </div>
                <div className="flex items-start">
                  <span className="text-green-500 mr-2 mt-1">✓</span>
                  <span className="text-sm text-gray-700">Target key resistance levels</span>
                </div>
                <div className="flex items-start">
                  <span className="text-red-500 mr-2 mt-1">×</span>
                  <span className="text-sm text-gray-700">Don't trade against strong trends</span>
                </div>
                <div className="flex items-start">
                  <span className="text-red-500 mr-2 mt-1">×</span>
                  <span className="text-sm text-gray-700">Avoid overtrading</span>
                </div>
              </div>
            </div>
          </div>
